...
namespace EonaCat.Blocky
{
    ...
    public static class Config
    {
        ...
        public static bool IncludeRawInLogTable = Configuration.GetValue("Fv", false);
        ...
        public static HashSet<BlockList> GetBlockySetup(bool forceReload = false)
        {
            if (forceReload || !File.Exists(BlockySetupSpec))
            {
                ...
                Logger.Log("Downloading blocky setup");
                var downloadSetup = DownloadBlockySetup();
                Logger.Log(downloadSetup
                    ? "The blocky setup was downloaded successfully"
                    : "The blocky setup could not be downloaded");
            }
            else
            {
                Logger.Log("The blocky setup already exists");
            }

            return ParseBlockySetup();
        }

        private static HashSet<BlockList> ParseBlockySetup()
        {
            ...
            foreach (var line in File.ReadLines(BlockySetupSpec))
            {
        ...
        private static bool DownloadBlockySetup()
        {
            var result = false;
            ...
            {
                Logger.Log($"Exception occurred during blocky setup download : {exception.Message}", ELogType.ERROR);
            }
