...
namespace EonaCat.Dns.Core
{
    public class Catalog : Dictionary<DomainName, DomainNode>
    {
        ...
        private void AddZone(MasterReader reader)
        {
            ...
            while ((resourceRecord = reader.ReadResourceRecord()) != null)
            {
        ...
        private void WithRootNs()
        {
            ...
            using (var hints = assembly.GetManifestResourceStream("EonaCat.Dns.Core.RootNS"))
            {
                ...
                {
                    AddOrUpdate(masterFileReader.ReadResourceRecord(rootHintEntry));
                }
            }
        ...
        public void WithResourceRecord(MasterReader reader, bool authoritative = false)
        {
            ResourceRecord resourceRecord;
            while ((resourceRecord = reader.ReadResourceRecord()) != null)
            {
        ...
        internal void AddReverseLookupRecord(AddressRecordBase addressRecord)
        {
            ...
            AddOrUpdate(ptrRecord, authoritative: true);
        }

        public static Catalog GenerateCatalog(string masterFileContents, bool withReverseLookup = true, bool withRootNs = true, bool withClientsFromDatabase = true)
        {
            var catalog = new Catalog();
            if (!string.IsNullOrWhiteSpace(masterFileContents))
            {
                catalog.AddZone(new MasterReader(new StringReader(masterFileContents)));
            }
