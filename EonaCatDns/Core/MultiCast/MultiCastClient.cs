    ...
    internal MultiCastClient(bool useIPv4, bool useIpv6, IEnumerable<NetworkInterface> nics)
    {
        ...
        foreach (var address in addresses)
        {
            ...
            try
            {
                ...
                LoggerMultiCast.LogAsync($"Will send via {localEndpoint}", ELogType.DEBUG).ConfigureAwait(false);
            ...
            {
                LoggerMultiCast.LogAsync($"Cannot setup send socket for {address}: {e.Message}", ELogType.ERROR)
        ...
        foreach (var receiver in _receivers) Listen(receiver);
    }
        ...
        foreach (var sender in _senders)
            try
            {
                var endpoint = sender.Key.AddressFamily == AddressFamily.InterNetwork
                    ? MultiCastDnsEndpointIp4
                    : MultiCastDnsEndpointIp6;
                await sender.Value.SendAsync(message, message.Length, endpoint).ConfigureAwait(false);
            }
            catch (Exception e)
            {
                await LoggerMultiCast.LogAsync($"Sender {sender.Key} failure: {e.Message}", ELogType.ERROR)
                    .ConfigureAwait(false);
            }
    }
