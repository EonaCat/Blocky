...
internal class ServiceDiscovery : IDisposable
{
        ...
        : this(new MultiCastService())
    {
        ...
        // Auto start.
        _ = MultiCastService.StartAsync().ConfigureAwait(false);
    }
    ...
    internal void Advertise(ServiceProfile service)
    {
        ...
        foreach (var subtype in service.Subtypes)
        {
            ...
            catalog.AddOrUpdate(ptr, true);
        }

        foreach (var r in service.Resources) catalog.AddOrUpdate(r, true);
    ...
    internal async Task AnnounceAsync(ServiceProfile profile)
    {
        ...
        await MultiCastService.SendAnswerAsync(message, false).ConfigureAwait(false);
    ...
    private async void OnAnswer(object sender, MessageEventArgs e)
    {
        await LoggerMultiCast.LogAsync($"Answer from {e.RemoteEndPoint} {e.Message}", ELogType.DEBUG)
    ...
    private async void OnQueryAsync(object sender, MessageEventArgs e)
    {
        ...
        await LoggerMultiCast.LogAsync($"Query from {e.RemoteEndPoint} {request}", ELogType.DEBUG)
            .ConfigureAwait(false);
        ...
        await LoggerMultiCast.LogAsync("Sending answer").ConfigureAwait(false);
        await LoggerMultiCast.LogAsync(response.ToString()).ConfigureAwait(false);
        await LoggerMultiCast.LogAsync($"Response time {(DateTime.Now - request.TimeCreated).TotalMilliseconds}ms",
            ELogType.DEBUG).ConfigureAwait(false);
    }
