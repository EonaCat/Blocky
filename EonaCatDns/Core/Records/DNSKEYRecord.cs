...
public class DnskeyRecord : ResourceRecord
{
        ...
        : this()
    {
        switch (algorithm)
        {
            ...
            default:
                throw new ArgumentException("EonaCatDns: " +
                                            $"Security algorithm '{algorithm}' is not allowed for a RSA key.");
        }
        ...
        : this()
    {
        ...
        {
            throw new ArgumentException("EonaCatDns: " +
                                        $"ECDSA curve '{parameters.Curve.Oid.FriendlyName} is not known'.");
        }
    ...
    public override void WriteData(MasterWriter writer)
    {
        ...
        writer.WriteByte(Protocol);
        writer.WriteByte((byte)Algorithm);
        writer.WriteBase64String(PublicKey, false);
    }
}