...
internal partial class Ns
{
    ...
    private void CleanExpiredRecords()
    {
        ...
        lock (_cacheLock)
        {
            foreach (var (question, resourceRecords) in _cache.ToList())
                _cache[question] = resourceRecords.Where(r => !(r.IsExpired || r.HasPacketError)).ToList();
        }
    }
    ...
    public async Task<Message> QueryAsync(Message request, CancellationToken cancel = default)
    {
        ...
        await Task.WhenAll(request.Questions.Select(question => QueryQuestionAsync(question, response, cancel)))
