...
namespace EonaCat.Dns.Core.Servers
{
    internal partial class Ns
    {
        ...
        private void CleanExpiredRecords()
        {
            ...
            foreach (var (question, resourceRecords) in _cache)
            {
                if (resourceRecords.Any(x => x.IsExpired || x.HasPacketError))
                {
                    recordsToRemove.Add(question);
                }
            }
            ...
            {
                _cache.TryRemove(recordToRemove, out _);
            }
        ...
        public async Task<Message> QueryAsync(Message request, CancellationToken cancel = default)
        {
            ...
            await Task.WhenAll(request.Questions.Select(question =>
                QueryQuestionAsync(question, response, cancel))).ConfigureAwait(false);
        ...
        protected internal void CacheAnswer(string question, List<ResourceRecord> records)
        {
            ...
            {
                _cache.TryAdd(question, records);
            }
        }
        ...
        private Task<bool> GetAnswerFromCatalogAsync(Question question, Message response, CancellationToken cancel)
        {
            ...
            response.Header.AuthoritativeAnswer |=
                (AuthoritativeAnswer)Convert.ToInt32(node.IsAuthoritative && question.Class != RecordClass.Any);

            var resources = node.Resources.Where(x =>
                (question.Class == RecordClass.Any || x.Class == question.Class) &&
                (question.Type == RecordType.Any || x.Type == question.Type)).ToList();
        ...
        protected async Task AddAdditionalRecordsAsync(Message response)
        {
            ...
            var totalAdditionalRecords = response.Answers.Count + response.AdditionalRecords.Count + response.AuthorityRecords.Count;
