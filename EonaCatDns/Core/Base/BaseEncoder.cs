...
namespace EonaCat.Dns.Core.Base
{
    public sealed class BaseEncoder : Encoder, ICryptoTransform
    {
        ...
        {
            Convert(chars, charIndex, charCount, bytes, byteIndex, bytes.Length - byteIndex, flush, out _, out var bytesUsed, out _);
            return bytesUsed;
        }
        ...
        {
            var outputBuffer = new byte[GetByteCount(inputBuffer, inputOffset, inputCount, flush: true)];
        ...
        public override unsafe void Convert(char* chars, int charCount, byte* bytes, int byteCount, bool flush, out int charsUsed, out int bytesUsed, out bool completed)
        {
            ...
            DecodeInternal(new ReadOnlySpan<char>(chars, charCount), new Span<byte>(bytes, byteCount), flush, out charsUsed, out bytesUsed, out completed);
        }
        ...
        public override void Convert(char[] chars, int charIndex, int charCount, byte[] bytes, int byteIndex, int byteCount, bool flush, out int charsUsed, out int bytesUsed, out bool completed)
        {
            ...
            DecodeInternal<char, byte>(chars.AsSpan(charIndex, charCount), bytes.AsSpan(byteIndex, byteCount), flush, out charsUsed, out bytesUsed, out completed);
        }
        ...
        public void Convert(byte[] chars, int charIndex, int charCount, byte[] bytes, int byteIndex, int byteCount, bool flush, out int charsUsed, out int bytesUsed, out bool completed)
        {
            ...
            DecodeInternal<byte, byte>(chars.AsSpan(charIndex, charCount), bytes.AsSpan(byteIndex, byteCount), flush, out charsUsed, out bytesUsed, out completed);
        }
        ...
        public void Convert(string chars, int charIndex, int charCount, byte[] bytes, int byteIndex, int byteCount, bool flush, out int charsUsed, out int bytesUsed, out bool completed)
        {
            ...
            DecodeInternal(chars.AsSpan(charIndex, charCount), bytes.AsSpan(byteIndex, byteCount), flush, out charsUsed, out bytesUsed, out completed);
        }
        ...
        private void DecodeInternal<TInputT, TOutputT>(ReadOnlySpan<TInputT> input, Span<TOutputT> output, bool flush, out int inputUsed, out int outputUsed, out bool completed) where TInputT : unmanaged where TOutputT : unmanaged
        {
            ...
            while (outputCount > 0)
            {
                ...
                if (typeof(TOutputT) == typeof(byte))
                {
                    ...
                    {
                        output[outputOffset + outputUsed + (outputSize - 1 - i)] = (TOutputT)(object)(byte)(outputBlock & 255);
                        outputBlock >>= 8;
                    }
                }
                if (typeof(TOutputT) == typeof(char))
                {
                    for (i = 0; i < outputSize; i++)
                    {
                        output[outputOffset + outputUsed + (outputSize - 1 - i)] = (TOutputT)(object)(char)(outputBlock & 255);
                        outputBlock >>= 8;
                    }
                }
        ...
        public override string ToString() => $"Base{Alphabet.Alphabet.Length}Encoder, {new string(Alphabet.Alphabet)}";
    }
}