...
{
    private unsafe void EncodeBase16(ReadOnlySpan<byte> input, Span<byte> output, bool flush, out int inputUsed,
        out int outputUsed, out bool completed)
    {
        ...
        fixed (byte* outputBytes = &MemoryMarshal.GetReference(output))
        fixed (byte* inputBytes = &MemoryMarshal.GetReference(input))
        {
    ...
    private unsafe void EncodeBase16(ReadOnlySpan<byte> input, Span<char> output, bool flush, out int inputUsed,
        out int outputUsed, out bool completed)
    {
        if (input.IsEmpty || output.IsEmpty)
        {
        ...
        fixed (char* alphabetPtr = Alphabet.Alphabet)
        fixed (char* outputBytes = &MemoryMarshal.GetReference(output))
        fixed (byte* inputBytes = &MemoryMarshal.GetReference(input))
        {
            var inputPtr = inputBytes;
            var outputPtr = outputBytes;
    ...
    private unsafe void EncodeBase32(ReadOnlySpan<byte> input, Span<byte> output, bool flush, out int inputUsed,
        out int outputUsed, out bool completed)
    {
        if (input.IsEmpty || output.IsEmpty)
        {
    ...
    private unsafe void EncodeBase32(ReadOnlySpan<byte> input, Span<char> output, bool flush, out int inputUsed,
        out int outputUsed, out bool completed)
    {
        if (input.IsEmpty || output.IsEmpty)
        {
    ...
    private unsafe void EncodeBase64(ReadOnlySpan<byte> input, Span<byte> output, bool flush, out int inputUsed,
        out int outputUsed, out bool completed)
    {
        if (input.IsEmpty || output.IsEmpty)
        {
    ...
    private unsafe void EncodeBase64(ReadOnlySpan<byte> input, Span<char> output, bool flush, out int inputUsed,
        out int outputUsed, out bool completed)
    {
        if (input.IsEmpty || output.IsEmpty)
        {
