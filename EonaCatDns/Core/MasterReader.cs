    ...
    public class MasterReader
    {
        ...
        public byte[] ReadResourceData()
        {
            ...
            while (stringBuilder.Length < length * 2)
            {
                ...
                if (word.Length % 2 != 0)
                {
                    throw new FormatException("EonaCatDns: " + $"The hex word ('{word}') must have an even number of digits.");
                }
        ...
        internal ResourceRecord ReadResourceRecord(RootHintEntry rootHintEntry = null)
        {
            ...
            while (!type.HasValue)
            {
                ...
                {
                    Logger.Log($"Could not load masterFile record with unknown class: {token}", ELogType.WARNING);
                }
                else
                {
                    if (!Enum.TryParse(token, ignoreCase: true, out RecordType t) || t == RecordType.Any)
                    {
                        throw new InvalidDataException("EonaCatDns: " + $"Unknown token '{token}', expected a Class, Type or TTL.");
                    }
        ...
        private ResourceRecord ReadResourceRecord(DomainName domainName, RecordType? type, RecordClass klass, TimeSpan? ttl, RootHintEntry rootHintEntry = null)
        {
