...
internal class StatsManager
{
        ...
        StatsType statsType, bool forceNew = false, bool isAuthenticated = false)
    {
        ...
        var statsDictionary =
            await GenerateStatsDictionaryAsync(statsCollection, isAuthenticated).ConfigureAwait(false);
        StatsCache.SetCache($"{statsType}", new Dictionary<string, List<StatsLog>>(statsDictionary),
    ...
    private static async Task<IDictionary<string, List<StatsLog>>> GenerateStatsDictionaryAsync(
        StatsCollection statsCollection, bool isAuthenticated = false)
    {
        ...
        data.Add(ConstantsDns.Stats.TopBlockedDomains,
            await statsCollection.GetTopBlockedDomainsAsync().ConfigureAwait(false));
    ...
    private static async Task UpdateStatsAsync(StatsCollection statsCollection)
    {
        ...
        {
            var hourStats =
                await LoadStatsForHourAsync(startDateTime.AddHours(i), startDateTime.AddHours(i + 1), statsCollection)
                    .ConfigureAwait(false);

            MergeStats(statsCollection.StatsTotal, hourStats.StatsByHour);
            AddStatsForDateTimeLabel(statsCollection, hourStats.StatsByHour,
                hourStats.StatsByHour.DateTime.ToString(statsCollection.LabelFormat));
        }
    }
    ...
    private static async Task<StatsPerHour> LoadStatsForHourAsync(DateTime startDateTime, DateTime endDateTime,
    ...
    private static List<StatsTotals> CalculateStatsTotalsPerMinute(DateTime startDateTime, List<Log> hourStatistics)
    {
        ...
        for (var i = 1; i < StatsPerHour.MinutesInHour; i++)
        {
            ...
            var statsTotal = new StatsTotals(startDatePerMinute)
            {
                ...
                TotalNameError =
                    statsListPerMinute.Count(x => x.ResponseCode == ResponseCode.NameError && !x.IsBlocked),
                ...
                TotalServerFailure =
                    statsListPerMinute.Count(x => x.ResponseCode == ResponseCode.ServerFailure && !x.IsBlocked)
            };
    ...
    private static void AddStatsForDateTimeLabel(StatsCollection statsCollection, StatsTotals statsCounter,
        string label)
    {
