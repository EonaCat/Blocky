...
public static class LoggerMultiCast
{
    ...
    public static async Task DownloadLogAsync(HttpContext context, string filePath)
    {
        ...
        try
        {
            ...
            while ((bytesRead = await fileStream.ReadAsync(buffer, 0, buffer.Length).ConfigureAwait(false)) > 0)
                ...
                {
                    await Logger.LogAsync("Cannot write to the response body after the response has completed.",
                        ELogType.WARNING);
        ...
        {
            await Logger.LogAsync($"Error downloading log file: {ex.Message}", ELogType.ERROR);
        }
        finally
        {
    ...
    public static async Task LogAsync(string message, ELogType logType = ELogType.INFO, bool writeToConsole = false)
    {
        if (IsDisabled)
        {
            return;
    ...
    public static async Task LogAsync(Exception exception, string message = "", bool writeToConsole = false)
    {
        if (IsDisabled)
        {
            return;
